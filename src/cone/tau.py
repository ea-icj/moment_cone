...
class Tau:
    ...
    @property
    def flattened(self) -> Sequence[int]:
        """
    ...
    def extend_from_S(self, p: Partition) -> "Tau":
        ...
        return Tau.from_flatten(my_list,LinearGroup([sum(p)]))
    
    def m_extend_with_repetitions(self,G: LinearGroup)-> Iterable["Tau"]:
        """
        ...
        >>> tau
        5 4 0 | 4 3 0 | 4 2 0
        >>> for t in tau.m_extend_with_repetitions(LinearGroup([4, 4, 3])):
        ...     print(t)
    ...
    def dim_Pu(self) -> int:
        ...
        >>> tau.dim_Pu
        8
        >>> sum(len(roots) for roots in tau.grading_rootsU.values())
        12
        """
        sum_di2 = self.G.dim
    ...
    def summands_Vtau(self,V : Representation)  -> list[list[int]] : 
        ...
        """
        #tau_red=Tau(self.reduced.values,LinearGroup([len(x) for x in self.reduced.values]))
        ...
        for i,w in enumerate(itertools.combinations(range(s+V.particle_cnt-1),s-1)) :
                L=[w[0]]
                for j,p in enumerate(itertools.pairwise(w)):
                    L.append(p[1]-p[0]-1)
                #Lself.G.rank+V.nb_part-w[-1]-2
                L.append(s+V.particle_cnt-w[-1]-2)
...
class ReducedTau:
    ...
    @property
    def small_d(self) -> LinearGroup:
        return LinearGroup(self.values.sizes)
...
def find_1PS(V: Representation, quiet: bool = False) -> list["Tau"]:
    ...
    if isinstance(V, KroneckerRepresentation):
        ...
        for partS in list_partS :
            ...
            Vred = type(V)(Gred, V.particle_cnt)
